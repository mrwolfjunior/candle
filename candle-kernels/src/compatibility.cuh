...
__device__ __forceinline__ __half atomicAdd(__half *address, __half val) {
    ...
    do {
        assumed = old;
        unsigned short int hsum = unaligned ? (old >> 16) : (old & 0xffff);
#if __CUDA_ARCH__ >= 530
        hsum = __half_as_ushort(__hadd(__ushort_as_half(hsum), val));
#else
        hsum = __half_as_ushort(__float2half(__half2float(__ushort_as_half(hsum)) + __half2float(val)));
#endif
        old = atomicCAS(address_as_ui, assumed,
            unaligned ? (old & 0xffff) | (((unsigned int)hsum) << 16) : (old & 0xffff0000) | (unsigned int)hsum
        );
